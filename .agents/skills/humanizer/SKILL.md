...
### 34. Ground every noun

**Words to watch:** canvas, shard, selector, paywall, fallback, backtick, approval, and any team-specific shorthand whose referent is not available to the reader.
**Problem:** The prose uses a noun that has no grounding. Nothing in the code has that name, and it is not an ordinary word in context. The reader can neither look it up nor infer it. This happens when the writer has just seen the referent in an editor and assumes everyone else can see it too.

Every noun must be one of three things: a code identifier in backticks, an ordinary word in the language being used, or a term explained at its first use. Avoid words that sit between these categories. They look like terms, but the reader has nowhere to find their meaning.

Before delivering text, ask of each noun: "Where can the reader find what this means?" If there is no answer, replace it with the exact code identifier or an ordinary word.

**Use borrowed terminology only when it is established or is a name from the code.** Do not turn internal English labels into unexplained slang when plain language exists. Write names from the interface or code exactly as they appear and put them in backticks rather than approximating or respelling them.
**Before:**
> The block looks successful in the chat response and on the canvas, but the paywall does not work.
**After:**
> The block appears in both the chat response and the editor page, but paid access does not work.

**Name the observable behavior, not an assumed mechanism.** Describe what someone can observe from outside. If an internal component matters, name it exactly in backticks rather than hinting at it.
**Before:**
> The value is outside the selector list.
**After:**
> The value does not appear in the list allowed for this property.

**A useful example names the object, field, and symptom.** Write examples so the reader can locate the relevant code or reproduce the case.
**Before:**
