...
function readCache(): CachedStats | null {
  try {
    ...
    if (
      ...
      parsed === null ||
      typeof (parsed as CachedStats).fetchedAt !== "number"
    ) {
      return null;
    }
...
function settledValue(result: PromiseSettledResult<string | null>) {
...
export default function HeroStatsComponent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const nextStars = settledValue(starsResult) ?? cached?.stars ?? null;
      const nextPulls = settledValue(pullsResult) ?? cached?.pulls ?? null;

      setStars(toStat(nextStars));
      setPulls(toStat(nextPulls));
      writeCache({ stars: nextStars, pulls: nextPulls, fetchedAt: Date.now() });
    };
